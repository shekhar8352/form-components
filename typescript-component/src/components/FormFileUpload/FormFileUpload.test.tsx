...
describe('FormFileUpload', () => {
  ...
  it('applies custom styling props', () => {
    ...
    // The component should render without errors when custom styling is applied
    expect(screen.getByRole('button', { name: /upload files/i })).toBeInTheDocument();
  });
  ...
  it('handles required field validation', () => {
    render(
      <TestWrapper>
        <FormFileUpload name="files" required={true} withAsterisk={true} />
      </TestWrapper>
    );

    // The component should render without errors when required
    expect(screen.getByRole('button', { name: /upload files/i })).toBeInTheDocument();
  });
});