...
function Example() {
  ...
  FormYesNoSelect: {
    ...
    defaultCode: `import { Formik, Form } from 'formik';
import { FormYesNoSelect } from 'reusable-form-components-ts';
import { Button, Stack } from '@mantine/core';
...
function Example() {
  const initialValues: FormValues = { isActive: '' };
...
function Playground() {
  const [selectedComponent, setSelectedComponent] = useState<string>('FormTextInput');
  const [code, setCode] = useState<string>(COMPONENT_TEMPLATES.FormTextInput.defaultCode);
  const [props, setProps] = useState<Record<string, any>>({});
  const [isTypeScript, setIsTypeScript] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  const handleComponentChange = useCallback((componentName: string | null) => {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <Stack gap="xl">
        <div>
          <Title order={1} mb="md">Interactive TypeScript Playground</Title>
          <Text size="lg" c="dimmed">
            Experiment with components in real-time with full TypeScript support
          </Text>
        </div>
          ...
          <Group justify="space-between" mb="md">
            <Group gap="md">
              ...
              <div>
                ...
                <Group gap="xs">
                  <Button
                    variant={!isTypeScript ? 'filled' : 'outline'}
                    size="xs"
        ...
        <Card shadow="sm" padding="lg" radius="md" withBorder>
          <Title order={3} mb="md">Generated TypeScript Code</Title>
          <CodeBlock 
