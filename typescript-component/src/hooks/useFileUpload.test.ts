...
describe('useFileUpload', () => {
  ...
  it('should call onFilesChange callback when files change', () => {
    const onFilesChange = vi.fn<[File[]], void>();
  ...
  it('should handle FileList input with proper typing', () => {
    const { result } = renderHook(() => useFileUpload());
  ...
  it('should maintain type safety with TypeScript', () => {
    ...
    const files: File[] = result.current.files;
    const uploading: boolean = result.current.uploading;
    const error: Error | null = result.current.error;
    const canAddMore: boolean = result.current.canAddMoreFiles;
    const remaining: number = result.current.remainingSlots;
    
    expect(Array.isArray(files)).toBe(true);
    expect(typeof uploading).toBe('boolean');
  ...
  it('should format different file sizes correctly', () => {
    ...
    testCases.forEach(({ size, expected }) => {
      ...
      const fileInfo = result.current.getFileInfo(file);
      expect(fileInfo.formattedSize).toBe(expected);
    });
  });
});