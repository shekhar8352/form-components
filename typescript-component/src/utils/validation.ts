...
export interface ValidationRule {
  type: 'required' | 'minLength' | 'maxLength' | 'pattern' | 'email' | 'custom' | 'fileSize' | 'fileType';
  value?: any;
  message: string;
...
export const formatValidationError = (error: string, fieldName?: string): string => {
  if (!error) return '';
...
export const validateEmail = (value: string, message: string = 'Please enter a valid email address'): ValidationResult => {
...
export const validateMinLength = (min: number, message?: string) => (value: string): ValidationResult => {
  if (!value) return undefined;
...
export const validateMaxLength = (max: number, message?: string) => (value: string): ValidationResult => {
  if (!value) return undefined;
...
export const validateNumber = (value: any, message: string = 'Must be a valid number'): ValidationResult => {
  if (!value) return undefined;
...
export const validateMinValue = (min: number, message?: string) => (value: any): ValidationResult => {
  if (!value) return undefined;
...
export const validateMaxValue = (max: number, message?: string) => (value: any): ValidationResult => {
  if (!value) return undefined;
...
export const validateFileSize = (maxSize: number, message?: string) => (file: File): ValidationResult => {
...
export const validateFileType = (allowedTypes: string[], message?: string) => (file: File): ValidationResult => {
  if (!file) return undefined;
...
export const validateMaxFiles = (maxFiles: number, message?: string) => (files: File[]): ValidationResult => {
...
export const validateMinDate = (minDate: Date, message?: string) => (value: Date | string): ValidationResult => {
  if (!value) return undefined;
...
export const validateMaxDate = (maxDate: Date, message?: string) => (value: Date | string): ValidationResult => {
  if (!value) return undefined;
...
export const createValidator = (rules: ValidationRule[]): ValidatorFunction => {
...
export const createYupValidator = (validator: ValidatorFunction) => {
